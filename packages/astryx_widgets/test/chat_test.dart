...
Widget wrap(Widget child) {
  ...
  return MaterialApp(theme: theme.light, home: Scaffold(body: Center(child: SizedBox(width: 520, child: child))));
}

void main() {
  testWidgets('AstryxChatMessage: user plain text + assistant markdown, with role semantics', (tester) async {
    await tester.pumpWidget(wrap(const Column(mainAxisSize: MainAxisSize.min, children: [
      AstryxChatMessage(role: AstryxChatRole.user, content: 'Hello'),
      AstryxChatMessage(role: AstryxChatRole.assistant, content: '# Reply'),
    ])));
  ...
  testWidgets('AstryxChatSystemMessage shows its notice', (tester) async {
    await tester.pumpWidget(wrap(const AstryxChatSystemMessage('New conversation')));
    expect(find.text('New conversation'), findsOneWidget);
  });

  testWidgets('AstryxChatToolCalls expands to reveal arguments and result', (tester) async {
    await tester.pumpWidget(wrap(const AstryxChatToolCalls(calls: [
      AstryxToolCall(name: 'web_search', status: AstryxToolStatus.success, arguments: '{"q":"astryx"}', result: '3 results'),
    ])));
  ...
  testWidgets('AstryxChatComposer sends on Enter, disables send when empty', (tester) async {
    String sent = '';
    ...
    expect(
      ...
      matchesSemantics(label: 'Send message', isButton: true, hasEnabledState: true),
    );
  ...
  testWidgets('AstryxChatLayout renders messages and the composer', (tester) async {
    await tester.pumpWidget(wrap(SizedBox(
      height: 400,
      child: AstryxChatLayout(
        messages: const [AstryxChatMessage(role: AstryxChatRole.user, content: 'Question?')],
